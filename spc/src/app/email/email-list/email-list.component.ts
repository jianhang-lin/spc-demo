...
export class EmailListComponent implements OnInit, AfterViewInit {
  ...
  enableActionsRules = emailsEnableActionsRules;
  availableTableActions = emailsTableActions;
  filterBySite;
  filterByPlant;
  filterByCustomer;
  ...
  getLoggedUserInfo() {
    ...
    this.enableActionsRules = this.is42qSite ? emailsEnableActionsRules42Q : emailsEnableActionsRulesSite;
    this.monitorGroupService.setIs42qSite(this.is42qSite);
    this.tableColumns = this.is42qAdmin && this.is42qSite ?
      emailsColumns42QAdmin : emailsColumnsSite;
  ...
  onGoToFunctionPage($event: MouseEvent) {
    this.confirmationService.confirm({
      icon: 'icon-info',
      message: this.translateService.instant('spc.emails-list.confirm-go-to-function-home-page-message'),
      accept: () => {
